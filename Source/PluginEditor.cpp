    ...
    browser(p.getAPVTS())
{
    ...
    setupS(ampAtkSlider, ampAtkLabel, "A"); setupS(ampDecSlider, ampDecLabel, "D");
    setupS(ampSusSlider, ampSusLabel, "S"); setupS(ampRelSlider, ampRelLabel, "R");

    setupS(modAtkSlider, modAtkLabel, "A"); setupS(modDecSlider, modDecLabel, "D");
    setupS(modSusSlider, modSusLabel, "S"); setupS(modRelSlider, modRelLabel, "R");
...
void LiquidDreamAudioProcessorEditor::resized()
{
    ...
    distDriveLabel.setBounds(sX + 10, sY + 30, 70, 20); distDriveSlider.setBounds(sX + 10, sY + 50, 70, 60);
    shpAmtLabel.setBounds(sX + 90, sY + 30, 70, 20); shpAmtSlider.setBounds(sX + 90, sY + 50, 70, 60);
    bitLabel.setBounds(sX + 170, sY + 30, 70, 20); bitSlider.setBounds(sX + 170, sY + 50, 70, 60);
    rateLabel.setBounds(sX + 250, sY + 30, 70, 20); rateSlider.setBounds(sX + 250, sY + 50, 70, 60);

    rightArea.removeFromTop(10);
    auto fltRect = rightArea.removeFromTop(120);
    filterGroup.setBounds(fltRect);
    int fX = fltRect.getX(), fY = fltRect.getY();
    cutoffLabel.setBounds(fX + 10, fY + 30, 70, 20); cutoffSlider.setBounds(fX + 10, fY + 50, 70, 60);
    resLabel.setBounds(fX + 90, fY + 30, 70, 20); resSlider.setBounds(fX + 90, fY + 50, 70, 60);
    fltEnvAmtLabel.setBounds(fX + 170, fY + 30, 70, 20); fltEnvAmtSlider.setBounds(fX + 170, fY + 50, 70, 60);

    rightArea.removeFromTop(10);
    ampEnvGroup.setBounds(rightArea);
    int eX = rightArea.getX(), eY = rightArea.getY();
    ampAtkLabel.setBounds(eX + 10, eY + 30, 60, 20); ampAtkSlider.setBounds(eX + 10, eY + 50, 60, 60);
    ampDecLabel.setBounds(eX + 80, eY + 30, 60, 20); ampDecSlider.setBounds(eX + 80, eY + 50, 60, 60);
    ampSusLabel.setBounds(eX + 150, eY + 30, 60, 20); ampSusSlider.setBounds(eX + 150, eY + 50, 60, 60);
    ampRelLabel.setBounds(eX + 220, eY + 30, 60, 20); ampRelSlider.setBounds(eX + 220, eY + 50, 60, 60);
}